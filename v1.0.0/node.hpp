...
class Node{
    ...
    std::string history;
    public:
    Node(LIST(Board) boards, int ill, std::string history){
        this->boards = boards;
    ...
    int expand(){
        ...
        if(this->boards[0].turn){
            ...
            for(const auto move:this->invalid_or_check_moves){
                LIST(Board) invalid_boards;
                std::unordered_map<b_info, LIST(Board)> valid_dict;
                for(auto board:this->boards){
                    if(board.is_legal(move)){
                        ...
                        valid_dict[info].push_back(pushed_board);
                    }else{
                        Board b(board);
                        invalid_boards.push_back(b);
                    };
                };
            ...
            std::unordered_map<w_info, LIST(Board)> info_dict;
    ...
    int search(int depth, std::stack<std::string> *answers){
