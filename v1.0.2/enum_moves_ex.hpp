...
inline int white_rider_king_from(char *board, limited_set<char, 100> *to_nums, int wk_pos){
...
inline std::unordered_set<Move> white_nondrop_evite(char *board, const limited_set<char, 100> to_nums, int wk_pos){
    ...
    while(n != 100){
        int p = board[n];
        if(p == W_PAWN){
            limited_set<char, 100> to_nums2;
            direction_S(n, &to_nums2);
            ITERATE_LIMITED_SET(i, to_nums2){
                if(to_nums.count(i)){
                    if(i < 90){
...
inline std::unordered_set<Move> white_legal_evite(char *board, unsigned char *hand, int wk_pos){
    std::unordered_set<Move> s;
    int i, j, n, a;
    unsigned char piece;
...
inline std::unordered_set<Move> preliminary_black_checks(char *board, const limited_set<char, 100> wk_nums){
    std::unordered_set<Move> s;
    int i, n;
    n = 11;
    while(n != 100){
        ...
        if(p == B_PAWN){
            ...
            ITERATE_LIMITED_SET(i, to_nums){
                if(wk_nums.count(i) || wk_nums.count(n)){
                    if(i > 20){
                        s.insert(make_move(n, i, NONPROMOTE, UNDEFINED));
                    };
                    if(i < 40){
...
inline std::unordered_set<Move> black_legal_prechecks(char *board, unsigned char *hand, int wk_pos){
    std::unordered_set<Move> s;
    unsigned char piece;
    int i, j, n, a;
    limited_set<char, 100> wk_nums;
    ...
    limited_set<char, 100> p;
    for(i = 0;i < 100;i++){
        if(board[i] == EMPTY && wk_nums.count(i)){
            p.insert(i);
        };
    };
    if(hand[0] != 0){
...
std::unordered_set<Move> black_check_moves(char *board, unsigned char *hand, int wk_pos){
