...
class Help{
    private:
        ...
        int ill;
        Board board;
        std::unordered_map<std::size_t, Board> board_data;
        std::unordered_map<std::size_t, int> ply_data;
        std::unordered_map<std::size_t, int> retry_data;
    public:
        Help();
        int main();
...
int Help::solve(){
    ...
    node.search(this->mate_ply, &answers, this->ans_lim, &this->ply_data, &this->retry_data, &this->board_data);
