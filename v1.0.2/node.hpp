...
class Node{
            ...
            for(auto move:pseudo_legal_moves){
                for(auto board:this->boards){
                    ...
                    if(board.is_legal(move)){
                        ...
                        b.push(move);
                        if(b.is_check()){
                            if(b.authentic_legal_moves().size()){
                                check_info = 1;
                            }else{
                                check_info = 2;
                            };
    ...
    int search(int depth, std::stack<std::string> *answers, int ans_lim, std::unordered_map<std::size_t, int> *ply_data, std::unordered_map<std::size_t, int> *retry_data, std::unordered_map<std::size_t, Board> *board_data){
        ...
        if(this->boards.size() == 1){
            for(auto board:this->boards){
                if(board.turn){
                    std::size_t x = board.hash_();
                    if(ply_data->count(x)){
                        if(board <= board_data->at(x)){
                            if(depth <= ply_data->at(x) && this->ill <= retry_data->at(x)){
                                return 0;
                            };
                        };
                    };
        ...
        if(depth){
            ...
            for(int i = 0;i < n;i++){
                ...
                if((node.is_simple()) && (node.turn())){
                    int ans_num = answers->size();
                    node.search(depth - 1, answers, ans_lim, ply_data, retry_data, board_data);
                    if(ans_num == answers->size()){
                        node.record_nomate(depth-1, ply_data, retry_data, board_data);
                    };
                }else{
                    node.search(depth - 1, answers, ans_lim, ply_data, retry_data, board_data);
                };
            ...
            for(int i = 0;i < n;i++){
                ...
                if((node.is_simple()) && (node.turn())){
                    int ans_num = answers->size();
                    node.search(depth, answers, ans_lim, ply_data, retry_data, board_data);
                    if(ans_num == answers->size()){
                        node.record_nomate(depth, ply_data, retry_data, board_data);
                    };
                }else{
                    node.search(depth, answers, ans_lim, ply_data, retry_data, board_data);
                };
    ...
    int record_nomate(int depth, std::unordered_map<std::size_t, int> *ply_data, std::unordered_map<std::size_t, int> *retry_data, std::unordered_map<std::size_t, Board> *board_data){
        for(auto board:this->boards){
            std::size_t x;
            x = board.hash_();
            ply_data->emplace(x, depth);
            retry_data->emplace(x, this->ill);
            board_data->emplace(x, board);
        };
        return 0;
    };
    bool turn(){
        for(const auto b:this->boards){
            return b.turn;
        };
        return false;
    };
